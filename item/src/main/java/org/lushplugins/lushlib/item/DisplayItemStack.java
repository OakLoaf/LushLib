...
public class DisplayItemStack {
    ...
    public ItemStack asItemStack(@Nullable Player player, boolean parseColors) {
        ...
        if (itemMeta != null) {
            if (displayName != null) {
                itemMeta.displayName(parseColors ? PaperColor.handler().translate(displayName, player) : Component.text(displayName));
            }

            if (lore != null) {
                itemMeta.lore(lore.stream()
                    .map(line -> parseColors ? PaperColor.handler().translate(line, player) : Component.text(line))
                    .toList());
            }
    ...
    public static Builder builder(ItemStack item) {
        ...
        if (itemMeta != null) {
            ...
            if (itemMeta instanceof SkullMeta) {
                itemBuilder.setSkullTexture(SkullCreator.getTextureValue(item));
            }
        }
    ...
    public static class Builder implements Cloneable {
        ...
        private Builder(@Nullable Material material) {
            this.material = material;
        }
        ...
        public Builder parseColors(Player player) {
            if (hasDisplayName()) {
                displayName = PaperColor.handler().translateRaw(displayName, player);
            }

            if (hasLore()) {
                lore = lore.stream().map(line -> PaperColor.handler().translateRaw(line, player)).toList();
            }
